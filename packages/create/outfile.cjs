...
// ../../node_modules/.pnpm/tsup@8.3.6_jiti@2.4.2_postcss@8.5.2_tsx@4.19.2_typescript@5.4.5_yaml@2.7.0/node_modules/tsup/assets/cjs_shims.js
...
function renderTemplate(src, dest, callbacks) {
  const stats = fs__namespace.statSync(src);
  if (stats.isDirectory()) {
    if (path2__namespace.basename(src) === "node_modules")
      return;
    fs__namespace.mkdirSync(dest, { recursive: true });
    for (const file of fs__namespace.readdirSync(src))
      renderTemplate(path2__namespace.resolve(src, file), path2__namespace.resolve(dest, file), callbacks);
    return;
  }
  const filename = path2__namespace.basename(src);
  if (filename === "package.json" && fs__namespace.existsSync(dest)) {
    const existing = JSON.parse(fs__namespace.readFileSync(dest, "utf8"));
    const newPackage = JSON.parse(fs__namespace.readFileSync(src, "utf8"));
    const pkg = sortDependencies(deepMerge_default(newPackage, existing));
    fs__namespace.writeFileSync(dest, `${JSON.stringify(pkg, null, 2)}
`);
    return;
  }
  if (filename === "tsconfig.json" && fs__namespace.existsSync(dest)) {
    const existing = JSON.parse(fs__namespace.readFileSync(dest, "utf8"));
    const newTsconfig = JSON.parse(fs__namespace.readFileSync(src, "utf8"));
    const tsconfig = deepMerge_default(existing, newTsconfig);
    fs__namespace.writeFileSync(dest, `${JSON.stringify(tsconfig, null, 2)}
`);
    return;
  }
  if (filename === "extensions.json" && fs__namespace.existsSync(dest)) {
    const existing = JSON.parse(fs__namespace.readFileSync(dest, "utf8"));
    const newExtensions = JSON.parse(fs__namespace.readFileSync(src, "utf8"));
    const extensions = deepMerge_default(existing, newExtensions);
    fs__namespace.writeFileSync(dest, `${JSON.stringify(extensions, null, 2)}
`);
    return;
  }
  if (filename === "settings.json" && fs__namespace.existsSync(dest)) {
    const existing = JSON.parse(fs__namespace.readFileSync(dest, "utf8"));
    const newSettings = JSON.parse(fs__namespace.readFileSync(src, "utf8"));
    const settings = deepMerge_default(existing, newSettings);
    fs__namespace.writeFileSync(dest, `${JSON.stringify(settings, null, 2)}
`);
    return;
  }
  if (filename === "_gitignore") {
    dest = path2__namespace.resolve(path2__namespace.dirname(dest), filename.replace(/^_/, "."));
  }
  if (filename.startsWith("_eslint")) {
    dest = path2__namespace.resolve(path2__namespace.dirname(dest), filename.replace(/^_(\w+)/, "$1"));
  }
  if (filename === "_gitignore" && fs__namespace.existsSync(dest)) {
    const existing = fs__namespace.readFileSync(dest, "utf8");
    const newGitignore = fs__namespace.readFileSync(src, "utf8");
    fs__namespace.writeFileSync(dest, `${existing}
${newGitignore}`);
    return;
  }
  if (filename.endsWith(".data.mjs")) {
    ...
    callbacks.push(async (dataStore) => {
      const getData = (await import(url.pathToFileURL(src).toString())).default;
...
// utils/banners.ts
init_cjs_shims();
var defaultBanner = "Quokka - Create Your First Quokka Project";
var gradientBanner = "\x1B[38;2;171;134;100mQ\x1B[39m\x1B[38;2;171;134;100mu\x1B[39m\x1B[38;2;171;134;100mo\x1B[39m\x1B[38;2;171;134;100mk\x1B[39m\x1B[38;2;172;136;104mk\x1B[39m\x1B[38;2;174;139;107ma\x1B[39m \x1B[38;2;175;141;111m-\x1B[39m \x1B[38;2;176;144;114mC\x1B[39m\x1B[38;2;177;146;118mr\x1B[39m\x1B[38;2;179;149;121me\x1B[39m\x1B[38;2;180;151;125ma\x1B[39m\x1B[38;2;181;154;129mt\x1B[39m\x1B[38;2;183;156;132me\x1B[39m \x1B[38;2;184;159;136mY\x1B[39m\x1B[38;2;185;161;139mo\x1B[39m\x1B[38;2;186;164;143mu\x1B[39m\x1B[38;2;188;166;147mr\x1B[39m \x1B[38;2;189;169;150mF\x1B[39m\x1B[38;2;190;171;154mi\x1B[39m\x1B[38;2;192;174;157mr\x1B[39m\x1B[38;2;193;176;161ms\x1B[39m\x1B[38;2;194;179;164mt\x1B[39m \x1B[38;2;196;181;168mQ\x1B[39m\x1B[38;2;197;184;172mu\x1B[39m\x1B[38;2;198;186;175mo\x1B[39m\x1B[38;2;199;189;179mk\x1B[39m\x1B[38;2;201;191;182mk\x1B[39m\x1B[38;2;202;194;186ma\x1B[39m \x1B[38;2;203;196;190mP\x1B[39m\x1B[38;2;205;199;193mr\x1B[39m\x1B[38;2;206;201;197mo\x1B[39m\x1B[38;2;207;204;200mj\x1B[39m\x1B[38;2;208;206;204me\x1B[39m\x1B[38;2;210;209;207mc\x1B[39m\x1B[38;2;211;211;211mt\x1B[39m";

// index.ts
var __dirname$1 = path2__namespace.default.dirname(url.fileURLToPath(importMetaUrl));
var argv = (0, import_minimist.default)(process2__default.default.argv.slice(2), { string: ["_"] });
var cwd = process2__default.default.cwd();
var FRAMEWORKS = [
  {
    name: "typescript",
    display: "Typescript",
...
var TEMPLATES = FRAMEWORKS.map(
  (f) => f.variants && f.variants.map((v) => v.name) || [f.name]
).reduce((a, b) => a.concat(b), []);
var renameFiles = {
  _gitignore: ".gitignore"
};
var defaultTargetDir = "quokka-project";
var MONOREPO_FILE = ["pnpm-workspace.yaml", "lerna.json"];
function isMonorepo(root) {
  const pkgPath = path2__namespace.default.join(root, "package.json");
  const pkgExist = fs__namespace.default.existsSync(pkgPath);
  return pkgExist && MONOREPO_FILE.some((file) => {
    return fs__namespace.default.existsSync(path2__namespace.default.join(root, file));
  });
}
var isProjectInMonorepo = function() {
  return isMonorepo(cwd) || isMonorepo(path2__namespace.default.resolve(cwd, ".."));
};
async function init() {
  console.log();
  console.log(
    process2__default.default.stdout.isTTY && process2__default.default.stdout.getColorDepth() > 8 ? gradientBanner : defaultBanner
  );
  console.log();
  const argTargetDir = formatTargetDir(argv._[0]);
  const argTemplate = argv.template || argv.t;
  let targetDir = argTargetDir || defaultTargetDir;
  const getProjectName = () => targetDir === "." ? path2__namespace.default.basename(path2__namespace.default.resolve()) : targetDir;
  let result;
  ...
  try {
    result = await (0, import_prompts.default)(
      [
        ...
        {
          type: () => !fs__namespace.default.existsSync(targetDir) || isEmpty(targetDir) ? null : "select",
          name: "overwrite",
        ...
        {
          name: "needsLint",
          type: (_, { monorepo: monorepo2 }) => monorepo2 ? null : "toggle",
  ...
  const root = path2__namespace.default.join(cwd, targetDir);
  if (overwrite === "yes")
    emptyDir(root);
  else if (!fs__namespace.default.existsSync(root))
    fs__namespace.default.mkdirSync(root, { recursive: true });
  const template = variant || (framework == null ? void 0 : framework.name) || argTemplate;
  const pkgInfo = pkgFromUserAgent(process2__default.default.env.npm_config_user_agent);
  const pkgManager = pkgInfo ? pkgInfo.name : "npm";
  const isYarn1 = pkgManager === "yarn" && (pkgInfo == null ? void 0 : pkgInfo.version.startsWith("1."));
  const { customCommand } = FRAMEWORKS.flatMap((f) => f.variants).find((v) => v.name === template) ?? {};
  if (customCommand) {
    const fullCustomCommand = customCommand.replace(/^npm create /, () => {
      if (pkgManager === "bun")
        return "bun x create-";
    ...
    const { status } = import_cross_spawn.default.sync(command, replacedArgs, {
      stdio: "inherit"
    });
    process2__default.default.exit(status ?? 0);
  }
  const templateRoot = path2__namespace.default.resolve(__dirname$1, "template");
  const callbacks = [];
  const render = function render2(templateName) {
    const templateDir = path2__namespace.default.resolve(templateRoot, templateName);
  ...
  for (const cb of callbacks)
    await cb(dataStore);
  if (monorepo) {
    const packageDir = path2__namespace.default.resolve(root, "packages", targetDir);
    render("monorepo");
    renderTemplate_default(
      path2__namespace.default.resolve(__dirname$1, `template-${template}`),
      packageDir,
      callbacks
    );
    renderTemplate_default(
      path2__namespace.default.resolve(templateRoot, "base"),
      packageDir,
      callbacks
    );
  } else {
    renderTemplate_default(
      path2__namespace.default.resolve(__dirname$1, `template-${template}`),
      monorepo ? path2__namespace.default.resolve(root, "packages", targetDir) : root,
      callbacks
    );
  }
  console.log(`
Scaffolding project in ${root}...`);
  const write = (file, content) => {
    const targetPath = path2__namespace.default.join(root, renameFiles[file] ?? file);
    if (content)
      fs__namespace.default.writeFileSync(targetPath, content);
  };
  const pkg = JSON.parse(
    fs__namespace.default.readFileSync(path2__namespace.default.join(root, `package.json`), "utf-8")
  );
  pkg.name = packageName || getProjectName();
  write(
    monorepo ? path2__namespace.default.join("packages", targetDir, "package.json") : "package.json",
    `${JSON.stringify(pkg, null, 2)}
`
  );
  const cdProjectName = path2__namespace.default.relative(cwd, root);
  console.log(`
Done. Now run:
`);
  if (root !== cwd) {
...
function isEmpty(path3) {
  const files = fs__namespace.default.readdirSync(path3);
  return files.length === 0 || files.length === 1 && files[0] === ".git";
}
function emptyDir(dir) {
  if (!fs__namespace.default.existsSync(dir))
    return;
  for (const file of fs__namespace.default.readdirSync(dir)) {
    if (file === ".git")
      continue;
    fs__namespace.default.rmSync(path2__namespace.default.resolve(dir, file), { recursive: true, force: true });
  }
}
